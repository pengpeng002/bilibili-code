...
point stack[LINES*LINES+20];//数组实现栈，方便遍历 
int stack_tail=0;//栈顶指针，指向栈顶元素的下一个 
...
void drawWindow(HDC &hdc)
{
    ...
    for (int i=0; i<stack_tail; i++)//直接从栈里读取下子顺序 
	{
		int px=stack[i].x, py=stack[i].y;
		if (i+1 == stack_tail)
		{
			HPEN pen = CreatePen(PS_SOLID, 2, RGB(0,0,255));
			SelectObject(hdc, pen);
		}
		SelectObject(hdc, (i%2==0) ? black : white);//黑棋先走，所以黑棋总是偶数，白棋奇数 
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	switch(Message) {
		case WM_PAINT: {
			printf("get message wm_paint\n");
			PAINTSTRUCT ps;
			BeginPaint(hwnd, &ps);//必不可少，不然MessageBox不会立刻弹出，原因不知道 
			if (useCache)//本来想用双缓冲，但屏幕刷新还是有问题，就没用了 
			{
		...
		case WM_LBUTTONDOWN: {//鼠标左键点击事件，下子 
			if (gameOver)
			{
				memset(maps, 0, sizeof(maps));
				gameOver = 0;
			...
			int mx = LOWORD(lParam), my = HIWORD(lParam);
			checkInGrid(mx, my);
			if (inGrid==1 && maps[px][py]==0)
			{
				maps[px][py] = nowBlack ? 1 : -1;
				stack[stack_tail++]={px, py};
				inGrid = 0;
				checkWin();
				if (gameOver && bWin)
				{
					...
					{
						MessageBox(hwnd, "end in tie!", "tips", 0);
					}
					redraw();		
					break;
				}
		...
		case WM_COMMAND: {
			switch (LOWORD(wParam))
			{
				...
				case ID_REACT: {
					if (!gameOver && stack_tail > 0)//双人对战只悔一子，人机对战多一子 
					{
						stack_tail--;
						maps[stack[stack_tail].x][stack[stack_tail].y]=0;
						if (useAI)
						{
							stack_tail--;
							maps[stack[stack_tail].x][stack[stack_tail].y]=0;
