		...
		for(auto x:ans)
		{
			...
			if(vn.find(s) != vn.end())//如果点后面式非终结符，形如A->X ·BY,则所有形如 B->X的产生式都应该属于这个闭包
			{
				for(auto i:yufa[s])
				{
					...
					if(ans.insert(make_pair(temp,1)).second)
					{
...
void go(int pos, string xx)
{
	...
	for(auto x:DFA[pos])
	{
		...
		{
			set<Obj> temp=closure(make_pair(x.first, x.second+1));//计算点向后移一位的闭包
...
void itemSets()
{
	set<Obj> statu=closure(make_pair(input[0],1));//初始项目的闭包，作为一个项目集
